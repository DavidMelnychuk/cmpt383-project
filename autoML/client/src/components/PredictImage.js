...
const PredictImage = ({ classNames }) => {
  ...
  const predictImage = (
    ...
    if (!classNames) {
      window.alert(
        "Error: Please upload zip files and train a model before predicting."
      );
      return;
    }
  ...
  return (
    <div id="predict-image-container">
      ...
      <div id="upload-predict-image">
        <form
          onSubmit={(e) =>
            predictImage(e, image, setPredictedLabel, setPredictionConfidence)
          }
          encType="multipart/form-data"
        >
          <input
            type="file"
            onChange={(e) => handleImageChange(e, setImage, setImageURL)}
            accept="image/*"
          ></input>
      ...
      {predictedLabel ? (
        <strong id="model-prediction">
          Model predicts: {predictedLabel} with {predictionConfidience} %
          confidence.
        </strong>
      ) : null}
      <LoadingOverlay spinner active={predicting} text="Predicting...">
        <div id="preview-image">
          {image ? (
            <img src={imageURL} alt="preview" width="150" height="150"></img>
          ) : null}
        </div>
      </LoadingOverlay>
    </div>
